...
class RegisterStateNotifier extends StateNotifier<RegisterState> {
  ...
  Future<void>register({required String email, required String password,required String name,required BuildContext context }) async {
    ...
    if(connected){
      ...
      response.when(
        success: (data) {
          state = state.copyWith(isLoading: false,errorMessage: null, isSuccess: true,isSocialSignIn:false,isNewUser: true);
        },
        failure: (error) {
    ...
    if(connected){
      ...
      response.when(
          success: (data) async{
            ...
            state = state.copyWith(isLoading: false,errorMessage: null,isSuccess: true,isSocialSignIn: true,isNewUser: data.user.isNewUser);
            if(context.mounted){
              AppHelpers.showSuccessToast(context, "Login Successful");
            }
          },
          failure: (error){
            state = state.copyWith(isLoading: false, errorMessage: NetworkExceptions.getErrorMessage(error), isSuccess: false,);
            if(context.mounted){
              AppHelpers.showErrorFlash(context, "Login Failed ${NetworkExceptions.getErrorMessage(error)}");
            }
          }
      );
    }else{
